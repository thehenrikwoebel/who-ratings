    ...
    <v-app>
      ...
      <v-main>
        <v-container>
          ...
          <EpisodeCard :classic="true" :episode="episodes[index]" :title="episodes_titles[index]" :description="episodes_descriptions[index]" :heart="episodes_hearts[index]" :rating="rating[index]" v-for="(item, index) in episodes_titles" v-bind:key="item.id"></EpisodeCard>
        </v-container>
      </v-main>
    </v-app>
  </template>
