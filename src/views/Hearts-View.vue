<template>
    ...
    <v-main>
        ...
        <v-container title="New Who">
            <EpisodeCard :season="seasons[index]" :rating="rating[index]" :classic="false" :episode="episodes[index]" :title="episodes_titles[index]" :description="episodes_descriptions[index]" :heart="episodes_hearts[index]" v-for="(item, index) in episodes_titles" v-bind:key="item.id"></EpisodeCard>
        </v-container>
        <v-divider></v-divider>
        <v-card-title class="text-h6 text-md-h5 text-lg-h4 text-center">Classic Who</v-card-title>
        <v-container title="Classic Who">
            <EpisodeCard :season="seasons_classic[index]" :rating="rating_classic[index]" :classic="true" :episode="episodes_classic[index]" :title="episodes_classic_titles[index]" :description="episodes_classic_descriptions[index]" :heart="episodes_classic_hearts[index]" v-for="(item, index) in episodes_classic_titles" v-bind:key="item.id"></EpisodeCard>
        </v-container>
    </v-main>
</template>
        ...
        data: () => ({
            ...
            async fetchClassicEpisodes() {
                ...
                for(let i = 0; i < eps.length;i++) { 
                    ...
                    this.seasons_classic[i] = eps[i].season;
                }
            }
        }),
        components: {
